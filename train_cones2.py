...
def main(args):
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("cones2", config=vars(args))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder):
                ...
                if num_iter == 0:
                    target_embed = encoder_hidden_states.detach()

                l_first_part = torch.norm(torch.squeeze(target_embed)[:1] - torch.squeeze(encoder_hidden_states)[:1], 2)
                l_second_part = torch.norm(torch.squeeze(target_embed)[1 + args.token_num:]
                                           - torch.squeeze(encoder_hidden_states)[1 + args.token_num:], 2)
    ...
    if accelerator.is_main_process:
        pipe = StableDiffusionPipeline.from_pretrained(args.pretrained_model_name_or_path).to("cuda")
        ...
        for template in PROMPT_TEMPLETE:
            text_inputs = pipe.tokenizer(
                template.format(args.instance_prompt),
                padding="max_length",
                max_length=tokenizer.model_max_length,
                truncation=True,
                return_tensors="pt",
            ...
            prompt_embeds_now = text_encoder(text_input_ids.to("cuda"), attention_mask=None)
            prompt_embeds_now = prompt_embeds_now[0][0][index_template: index_template + args.token_num]
            prompt_embeds = pipe.text_encoder(text_input_ids.to("cuda"), attention_mask=None)
            prompt_embeds = prompt_embeds[0][0][index_template: index_template + args.token_num]
            prompt_embeds_new += prompt_embeds_now
            prompt_embeds_origin += prompt_embeds

        residual_save_path = args.output_dir + '/residual.pt'
        ...
        accelerator.print(f"The customized residual embedding saved in {residual_save_path}")
